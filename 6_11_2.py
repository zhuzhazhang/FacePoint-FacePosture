...
def add_layer(inputs, in_size, out_size, activation_function=None):
    ...
    weights = tf.Variable(tf.truncated_normal([in_size, out_size], stddev=10))
...
l1 = add_layer(xs, 10, 32, activation_function=tf.nn.sigmoid)
l2 = add_layer(l1, 32, 16, activation_function=tf.nn.sigmoid)
l3 = add_layer(l2, 16, 8, activation_function=tf.nn.sigmoid)
l4 = add_layer(l3, 8, 4, activation_function=tf.nn.sigmoid)
# l5 = add_layer(l4, 4, 3, activation_function=tf.nn.sigmoid)
...
prediction = add_layer(l4, 4, 3, activation_function=None)
...
sess.run(init)
# writer = tf.summary.FileWriter("E:/szu/graph", sess.graph)
loss_num = []
...
for i in range(20001):
    ...
    if i % 100 == 0:
        ...
        # print(sess.run(l7, feed_dict={xs: x_train, ys: y_train}))
        print('layer:%d' % i, sess.run(loss, feed_dict={xs: x_train, ys: y_train}))
    if i % 1000 == 0:
        saver.save(sess, './6_11_2/Model.ckpt', global_step=i)
